...
class ControllerSaleOrder extends Controller {
	...
	public function filterTelephone($s) {
	    $a['-'] = '';
	    $a[' '] = '';
	    $a['('] = '';
	    $a[')'] = '';
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['date_added'] = date('j.m.Y G:i', strtotime($order_info['date_added']));
			...
			$data['email'] = $order_info['email'];
			$data['telephone'] = $this->filterTelephone($order_info['telephone']);
			$data['telephone_for_viber'] = str_replace('+','%2B',$data['telephone']);
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'href_front'       => HTTPS_CATALOG .'index.php?route=product/product&product_id='.$product['product_id']
				);
			}
