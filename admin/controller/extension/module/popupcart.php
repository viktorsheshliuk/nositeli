...
//$s = json_encode($this->request->post); 
//var_dump($this->request->post);die();
//$this->config->set('popupcart1', $s);	
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('popupcart', $this->request->post);
			} else {
			...
			if ($this->request->post['apply']) {
				$url = $this->url->link('extension/module/popupcart', 'user_token=' .  $this->session->data['user_token'], 'SSL');
			} else {
				$url = $this->url->link('marketplace/extension', 'user_token='. $this->session->data['user_token'] .'&type=module', 'SSL');
			}
		...
		$data['error_warning'] = isset($this->error['warning']) ? $this->error['warning'] : '';		
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('extension/module/popupcart', 'user_token=' . $this->session->data['user_token'], true),
   		);

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/popupcart', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/popupcart', 'user_token=' . $this->session->data['user_token']. '&module_id=' . $this->request->get['module_id'], true);
		}
...
//echo "<pre>";var_dump($module_info);echo"</pre>";
		$data['settings'] = $module_info['popupcart'];
//echo "<pre>";var_dump($data['settings']);echo"</pre>";die();			
