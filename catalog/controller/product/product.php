...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$formcreator = new formcreator($this->registry); $data['formcreator_id46'] = $formcreator->initFeedback(46);
			...
			$this->document->setProductId($product_id); // динамический ремаркетинг
			...
			$data['ostatok'] = $product_info['quantity'];
			$data['ean'] = $product_info['ean'];
			$data['viewed'] = $product_info['viewed'];
			$data['main_image_width'] = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_thumb_width');
			$data['main_image_height'] = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_thumb_height');
			$data['additional_image_width'] = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_additional_width');
			$data['related_image_width'] = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width');
			$data['related_image_height'] = $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height');
			...
			if ((float)$product_info['special']) {
				...
				if ($data['price'] > $data['special'] || $data['price'] !== 0){
					$data['discount_percent'] = '-'. round(($data['price'] - $data['special']) / $data['price'] * 100) . '%';
				}	
			} else {
				$data['special'] = false;
			}
			...
			foreach ($results as $result) {
				if (($result['stock_status_id'] == '9')&&($result['quantity'] <= 0)) continue;	
				...
				if ((float)$result['special']) {
					...
					$special = str_replace(".00","",$special); //удаление нолей после запятой в цене товара 
				} else {
					$special = false;
				}
