...
class ControllerExtensionFeedUnixml extends Controller {
            ...
            foreach($products as $product_id => $product){
              ...
              if(isset($product['weight']) && isset($product['weight_class_id'])){
                $xml .= '<g:weight>' . $this->weight->format($product['weight'], $product['weight_class_id']) . '</g:weight>';
  						...
  						$xml .= '<g:availability>' . ($product['quantity'] ? 'in_stock' : 'out_of_stock') . '</g:availability>';
              foreach($product['attributes_full'] as $attribute){
                $xml .= '<' . $attribute['name'] . '>' . $attribute['text'] .  '</' . $attribute['end'] . '>';
              }
              foreach($product['attributes'] as $attribute){
  ...
  //5 google xml
public function google_copy() {
    $this->feed = 'google_copy';
    $xml = false;
    $startup = $this->startup();
          ...
          for($i=0; $i<10000000/$this->step; $i++){
            ...
            if($products){
              foreach($products as $product_id => $product){
                ...
                if(isset($product['car'])){
                  $xml .= '<compatibility><car>' . $product['car'] . '</car></compatibility>';
                }
                $xml .= '<manufacturer>' . $product['manufacturer'] .  '</manufacturer>';
                $xml .= '<description><![CDATA[' . $product['description'] .  ']]></description>';
                if($product['special']){
                ...
                $xml .= '<availability><isAvailable>' . ($product['stock']?'True':'False') .'</isAvailable></availability>';
                $xml .= '<images>';
                $xml .= '<image>' . $product['image'] .  '</image>';
                if($product['images']){
                  foreach($product['images'] as $image){
