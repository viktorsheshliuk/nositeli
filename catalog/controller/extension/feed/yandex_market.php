	...
	public function index() {
		if ($this->config->get('feed_yandex_market_status')) {
			...
			foreach ($products as $product) {
				$data = array();
				if ($product['quantity'] <= 0){ continue;} //выводим только товар что есть в наличии 
...
*/
			...
			$filename = 'temp/yandex-market.xml';
			file_put_contents($filename, $this->getYml(), LOCK_EX);
	...
	private function setOffer($data) {
		...
		$allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'wprice'=>0, 'currencyId'=>1, 'xCategory'=>0, 'categoryId'=>1, 'picture'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0, 'location'=>1);
