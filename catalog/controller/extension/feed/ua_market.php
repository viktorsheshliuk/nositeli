...
class ControllerExtensionFeedUaMarket extends Controller {
	...
	public function index() {
		if ($this->config->get('feed_ua_market_status')) {
			...
			// Защитный ключ
			$secret_key = $this->config->get('feed_ua_market_secret_key');
			...
			$allowed_categories = $this->config->get('feed_ua_market_categories');
			$allowed_manufacturers = $this->config->get('feed_ua_market_manufacturers');

			//if (!$allowed_categories && !$allowed_manufacturers) exit();

			$this->load->model('extension/feed/ua_market');
			$this->load->model('localisation/currency');
			$this->load->model('tool/image');

			// Магазин
			$this->setShop('name', $this->config->get('feed_ua_market_shopname'));
			// $this->setShop('company', $this->config->get('feed_ua_market_company'));
			...
			$offers_currency = $this->config->get('feed_ua_market_currency');
			...
			// Категории
			$categories = $this->model_extension_feed_ua_market->getCategory();
			...
			$bus_id = $this->config->get('feed_ua_market_id'); // Идентификатор товара - "id"
			$bus_type = $this->config->get('feed_ua_market_type'); // Тип предложений - "type"
			$bus_name = $this->config->get('feed_ua_market_name'); // Название товара - "name"
			$bus_model = $this->config->get('feed_ua_market_model'); // Код товара - "model"
			$bus_vendorCode = $this->config->get('feed_ua_market_vendorCode'); // Артикул товара - "vendorCode"

			$bus_image = $this->config->get('feed_ua_market_image'); // Статус товара без изображений
			$bus_image_width = $this->config->get('feed_ua_market_image_width'); // Ширина изображения товара
			$bus_image_height = $this->config->get('feed_ua_market_image_height'); // Высота изображения товара
			$bus_image_quantity = $this->config->get('feed_ua_market_image_quantity'); // Количество изображений товара

			$bus_main_category = $this->config->get('feed_ua_market_main_category'); // Статус товара без главной категории

			$in_stock_id = $this->config->get('feed_ua_market_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('feed_ua_market_out_of_stock'); // id статуса товара "Нет на складе"
			$bus_quantity_status = $this->config->get('feed_ua_market_quantity_status'); // Статус товара "количество равное 0"

			$vendor_required = false; // true - только товары у которых задан производитель, необходимо для 'vendor.model'

			$products = $this->model_extension_feed_ua_market->getProduct($allowed_categories, $allowed_manufacturers, $out_of_stock_id, $vendor_required, $bus_image, $bus_image_quantity, $bus_main_category, $bus_quantity_status);
...
//				$data['local_delivery_cost'] = 100;
				...
				if (!empty($product[$bus_model])) {
					$data['article'] = $product[$bus_model];
				} else {
					$data['article'] = '';
				}
				if (!empty($product['description'])) {
					$data['description_ua'] = $product['description'];
				} else {
					$data['description_ua'] = '';
				}
...
*/
	...
	private function setOffer($data) {
		...
		switch ($type) {
			...
			default:
				$allowed_tags = array_merge($allowed_tags, array('name'=>1, 'name_ua'=>0, 'description_ua'=>0, 'vendor'=>0, 'vendorCode'=>0, 'article'=>0,'quantity_in_stock'=>0, 'model'=>1, 'available'=>0));
				break;
		}
		...
		foreach ($tags as $key => $value) {
			if ($key == 'picture') {
				foreach (explode(',', $value) as $val) {
					$retval .= '<picture>' . $val . '</picture>' . $this->eol;
				}
