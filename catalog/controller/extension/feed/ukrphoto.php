...
class ControllerExtensionFeedUkrPhoto extends Controller {
	...
	public function index() {
		if ($this->config->get('feed_ukrphoto_status')) {

			// Язык фида

			$code = 'uk-ua'; //язык по умолчанию
			...
			// Защитный ключ
			$secret_key = $this->config->get('feed_ukrphoto_secret_key');
			...
			$allowed_categories = $this->config->get('feed_ukrphoto_categories');
			$allowed_manufacturers = $this->config->get('feed_ukrphoto_manufacturers');

			//if (!$allowed_categories && !$allowed_manufacturers) exit();

			$this->load->model('extension/feed/ukrphoto');
			$this->load->model('localisation/currency');
			$this->load->model('tool/image');
			$this->load->model('catalog/product');

			// Магазин
			$this->setShop('name', $this->config->get('feed_ukrphoto_shopname'));
			$this->setShop('company', $this->config->get('feed_ukrphoto_company'));
			...
			$offers_currency = $this->config->get('feed_ukrphoto_currency');
			...
			// Категории
			$categories = $this->model_extension_feed_ukrphoto->getCategory();
			...
			$bus_id = $this->config->get('feed_ukrphoto_id'); // Идентификатор товара - "id"
			$bus_type = $this->config->get('feed_ukrphoto_type'); // Тип предложений - "type"
			$bus_name = $this->config->get('feed_ukrphoto_name'); // Название товара - "name"
			$bus_model = $this->config->get('feed_ukrphoto_model'); // Код товара - "model"
			$bus_vendorCode = $this->config->get('feed_ukrphoto_vendorCode'); // Артикул товара - "vendorCode"

			$bus_image = $this->config->get('feed_ukrphoto_image'); // Статус товара без изображений
			$bus_image_width = $this->config->get('feed_ukrphoto_image_width'); // Ширина изображения товара
			$bus_image_height = $this->config->get('feed_ukrphoto_image_height'); // Высота изображения товара
			$bus_image_quantity = $this->config->get('feed_ukrphoto_image_quantity'); // Количество изображений товара

			$bus_price_nacenka = $this->config->get('feed_ukrphoto_price_nacenka'); // Наценка для выгружаемых цен

			$bus_main_category = $this->config->get('feed_ukrphoto_main_category'); // Статус товара без главной категории

			$in_stock_id = $this->config->get('feed_ukrphoto_in_stock'); // id статуса товара "В наличии"
			$out_of_stock_id = $this->config->get('feed_ukrphoto_out_of_stock'); // id статуса товара "Нет на складе"
			$bus_quantity_status = $this->config->get('feed_ukrphoto_quantity_status'); // Статус товара "количество равное 0"

			$vendor_required = false; // true - только товары у которых задан производитель, необходимо для 'vendor.model'

			$products = $this->model_extension_feed_ukrphoto->getProduct($allowed_categories, $allowed_manufacturers, $out_of_stock_id, $vendor_required, $bus_image, $bus_image_quantity, $bus_main_category, $bus_quantity_status);
			$products_ua = $this->model_extension_feed_ukrphoto->getProduct_UA($allowed_categories, $allowed_manufacturers, $out_of_stock_id, $vendor_required, $bus_image, $bus_image_quantity, $bus_main_category, $bus_quantity_status);
...
//				$data['cbid'] = 15;
				...
				if (isset($product['ps_price'])){
					$data['clear_price'] = number_format($this->currency->convert($this->tax->calculate($product['ps_price'], $product['tax_class_id']), $shop_currency, $offers_currency), $decimal_place, '.', '');
				} else {
					$data['clear_price'] = $data['price']; 
				}
...
*/
				...
				if(($data['vendor'] == 'Evg')||($data['vendor'] == 'Chako')||($data['vendor'] == 'Ufo')||($data['vendor'] == 'BookBound')||($data['vendor'] == 'Nobrand')||($data['vendor'] == 'Poldom')||($data['vendor'] == 'Gedeon')){
					$data['param'][] = array(
						'name' => 'Страна-виробник',
						'value' => 'Китай'
					); 
				}
				foreach ($data['param'] as $key_param => $tmp_param ) {
					if ($tmp_param['name'] == 'Кількість сторінок'){
						$data['param'][] = array(
						'name' => 'Кількість листів',
						'value' => $tmp_param['value'] /2
					);  	
					}
					if ($tmp_param['name'] == 'Тип кріплення фото'){
						$data['param'][$key_param]['name'] = 'Тип кріплення фото';
						switch ($tmp_param['value']) {
							case 'С кишенями 10см*15см':
								$data['param'][$key_param]['value'] = 'Плівкова кишеня';
								//ключевые слова для категорий фотоальбомов
								$data['keywords'] .=',Фотоальбом з кишеньками, Фотоальбом для фотографій 10*15';  
								break;
							case 'С кишенями 13см*18см':
								$data['param'][$key_param]['value'] = 'Плівкова кишеня';
								$data['keywords'] .=',Фотоальбом з кишеньками, 13x18, 13 * 18, Фотоальбоми 13х18'; 
								break;
							case 'С кишенями 15см*21см':
								$data['param'][$key_param]['value'] = 'Плівкова кишеня';
								$data['keywords'] .=',Фотоальбом з кишеньками, 15x21,15*21'; 
								break;	
							case 'С магнітною плівкою':
								$data['param'][$key_param]['value'] = 'Магнітні сторінки';
								$data['keywords'] .=',Фотоальбом з магнітною плівкою, самоклеючий фотоальбом, Самоклеючі фотоальбоми'; 
								break;
							case 'На куточки або скотч':
								$data['param'][$key_param]['value'] = 'Двосторонній скотч';
								$data['keywords'] .=',традиційний фотоальбом, фотоальбом на куточки, фотоальбом на скотч, Фотоальбоми Традиційні'; 
								break;
						}
										  	
					}
					if ($tmp_param['name'] == 'Матеріал обкладинки'){
						//$data['param'][$key_param]['name'] = 'Тип крепления';
						switch ($tmp_param['value']) {
							case 'Кожзам':
								$data['param'][$key_param]['value'] = 'Штучна шкіра';
								break;
						}
	...
	private function setOffer($data) {
		...
		switch ($type) {
			...
			$allowed_tags = array_merge($allowed_tags, array('name'=>1, 'name_ua'=>0, 'vendor'=>0, 'vendorCode'=>0, 'model'=>1, 'keywords'=>0 ,'keywords_ua'=>0, 'ean'=>0,'clear_price'=>0, 'date_available'=>0, 'quantity_in_stock'=>0, 'minimum'=>0));
				break;
		}
