...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['telephone_for_viber'] = str_replace('+','%2B',$order_info['telephone']);
		...
		foreach ($order_products as $order_product) {
			...
			if ($order_product['image']) {
					$my_image = $this->model_tool_image->resize($order_product['image'], 40, 40);
				} else {
					$my_image = $this->model_tool_image->resize('placeholder.png', 40, 40);
				}
		...
		if ($order_info['email'] != 'Не указано'){
  ...
  //       $page->getColumnDimensionByColumn("1")->setWidth(60);
  //       $page->getColumnDimensionByColumn("2")->setWidth(10);
  //       $page->getColumnDimensionByColumn("3")->setWidth(11);
  //       $page->getColumnDimensionByColumn("4")->setWidth(10);
  //       $page->getColumnDimensionByColumn("5")->setWidth(10); //задаем ширину столбцов
        
  //       $page->setCellValue("A1", "");
  //       $page->setCellValue("B1", "Наименование");
  //       $page->setCellValue("C1", "Модель");
  //       $page->setCellValue("D1", "Количество"); //прописали в первой строке название столбцов
  //       $page->setCellValue("E1", "Цена");
		// $page->setCellValue("F1", "Всего");
        ...
        $i=2;
         foreach($order_products as $product){ //перебираем массив продуктов и записываем в файл
            
           $sheet->setCellValue("B$i", $product['name']);
           $sheet->setCellValue("C$i", $product['model']);
           $sheet->setCellValue("D$i", $product['quantity']);
           $sheet->setCellValue("E$i", $product['price']);
           $sheet->setCellValue("F$i", $product['total']);
           $i++;            
       }
        
       $sheet->setTitle("Товары");
            
       $filename = DIR_SYSTEM . 'products.xls';
        ...
        ///////////////////////EXCEL
			...
			$mail->setSender(html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
			//краткая тема письма:
			//$s = $this->config->get('config_name');
			//$tempArr=explode(' ',$s,2);
 			$mail->setSubject(html_entity_decode(sprintf($this->language->get('text_subject'), 'Носители', $order_info['order_id'], $order_info['lastname']), ENT_QUOTES, 'UTF-8'));
			
 			$mail->addAttachment($filename);

			$mail->setHtml($this->load->view('mail/my_order_add', $data));     //было 'mail/order_alert', я подставил свой шаблон вывода 
			...
			foreach ($emails as $email) {
				if ($email && filter_var($email, FILTER_VALIDATE_EMAIL)) {
					$mail->setTo($email);	
				if (!isset($this->session->data['api_id']))   $mail->send();// $mail->send();
				}
			}
		}
	}
