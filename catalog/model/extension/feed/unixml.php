...
// }  
public function getParentCategories($category_id, $categories = array(), $lang) {
 // var_dump((int)$lang);die();
  //$this->feed = 'google';
 // var_dump(${$this->varname('language')});die();
    ...
    if ($query->num_rows) {
        ...
        if ($parent_id == 0) {
            ...
            $parent_query = $this->db->query("SELECT category_id, name FROM " . DB_PREFIX . "category_description WHERE category_id = '" . (int)$parent_id . "' AND language_id = '" . (int)$lang . "' ");
            
            if ($parent_query->num_rows) {
                $categories[] = array(
                    'category_id' => $parent_query->row['category_id'],
                    'name' => $parent_query->row['name']
                );
                
                return Self::getParentCategories($parent_id, $categories, $lang);
            }
        }
    }
...
//$category_id = 10; // Замените на нужный category_id
//$parent_categories = getParentCategories($category_id);

// Вывод названий родительских категорий
//foreach ($parent_categories as $parent_category) {
//    echo $parent_category['name'] . "<br>";
//}
  //getProducts
...
$categories_text = '';
$arr = array();
$parent_categories = $this->getParentCategories($product['category_id'], $arr, (int)$lang);

foreach ($parent_categories as $parent_category) {
     $categories_text .= $parent_category['name'] . " > ";
}        

$product['categories_text'] = $categories_text . $product['category'];              
...
if($feed == "google_copy"){ //google_copy_custom_code_xml_start

} //google_copy_custom_code_xml_end
